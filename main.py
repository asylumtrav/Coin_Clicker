...
def format_click_power(cp):
    if cp < 10:
        return f"{cp:.1f}"
    else:
        return format_large_number(cp)
...
# --- Upgrade Data ---
click_upgrades = [
    {"name": "Pixel Puncher", "base_cost": 10, "click_power": 0.1, "owned": 0, "max_owned": 1000000, "image_path": os.path.join(ASSETS_DIR, "punche.png")},
...
class GameState:
    def __init__(self):
        ...
        self.auto_upgrades = []
        self.cp_multipliers = []   # list of dicts, each with "boost_percent" and "purchased"
        self.cps_multipliers = []
        self.click_upgrades = []
        for u in click_upgrades:
            copy_u = dict(u)
            copy_u["base_cost_initial"] = copy_u["base_cost"]
            copy_u["flash_timer"] = 0      # <-- Add here!
    ...
    def get_total_click_power(self):
        multiplier = 1.0 + 0.01 * self.prestige
        total = self.base_click_power
        for upg in self.click_upgrades:
            total += upg["click_power"] * upg["owned"]
        return total * multiplier

    def get_total_cps(self):
        total = 0
        for upg in self.auto_upgrades:
            total += upg["cps"] * upg["owned"]
        return total
    ...
    def buy_upgrade(self, upgrade, levels=1):
        ...
        if self.coins >= total_cost and (start_level + levels) <= upgrade["max_owned"]:
            ...
            upgrade["flash_timer"] = 0.25  # <--- This is the correct place!
        ...
        for idx, upg in enumerate(upgrade_list):
            ...
            for tier, owned_required in enumerate(unlocks):
                ...
                all_multipliers.append({
                    ...
                    "image_path": os.path.join(ASSETS_DIR, f"{base_name.lower().replace(' ', '').replace(':', '')}.png")
                })
...
def draw_stats_screen(surface, window_width, window_height, state, big_font, font, mouse_pos, mouse_down):
    ...
    cp_base = state.base_click_power
    cp_total = state.get_total_click_power()
    if cp_total > cp_base:
        cp_boost_percent = round(100 * (cp_total - cp_base) / cp_base, 1)
    else:
        cp_boost_percent = 0

    cp_upgrade_total = sum(upg["click_power"] * upg["owned"] for upg in state.click_upgrades)
    cps_upgrade_total = sum(upg["cps"] * upg["owned"] for upg in state.auto_upgrades)

    lines = [
        f"Total Coins Earned This Prestige: {format_large_number(state.prestige_earned)}",
        f"Total Coins Earned (All Time): {format_large_number(state.all_time_coins)}",
        ...
        f"Click Power (CP): {format_click_power(state.get_total_click_power())}",
        f"CPS: {format_large_number(state.get_total_cps())}",
        f"Total Clicks: {state.total_clicks}",
        f"CP From Upgrades: {format_click_power(cp_upgrade_total)}",
        f"CPS From Upgrades: {format_large_number(cps_upgrade_total)}",
        ]
...
def main():
    ...
    while True:
            ...
            if event.type == pygame.QUIT:
                ...
                if event.button in (1, 3):
                    ...
                    if showing_achievements:
                        ...
                        visible_height = window_height - (tab_y + tab_height + 20 + 90)  # accounts for tabs + shop boxes
        ...
        state.prestige_earned += gained_cps

        # --- Draw Stats, Achievements (with notif dot), and Settings buttons aligned bottom-left ---
        if not showing_stats and not showing_settings and not showing_achievements and state.active_tab != "PRESTIGE":
            padding = 10
        ...
        draw_text(screen, f"Click Power: {format_click_power(state.get_total_click_power())}", (20, 50), FONT)
        ...
        if not showing_stats and not showing_settings and not showing_achievements:
            ...
            if pygame.mouse.get_pressed()[0]:
                if state.active_tab != "PRESTIGE":
                    ...
                    # On Prestige: Only allow "X" or "Prestige Now" to exit
                    pass  # Do not allow switching tabs while on Prestige
            ...
            if state.active_tab in ("CP", "CPS"):
                ...
                # --- Contain the upgrade list between tabs and multiplier buttons ---
                multiplier_btn_height = max(30, int(window_height * 0.06))
                multiplier_btn_margin = 50
                bottom_margin = -10
                base_y = window_height - multiplier_btn_height - 20  # 20 is typical bottom margin
                upgrade_area_height = window_height - start_y - multiplier_btn_height - multiplier_btn_margin - bottom_margin

                clip_rect = pygame.Rect(left_width, start_y, right_width, upgrade_area_height)
                screen.set_clip(clip_rect)

                # === DRAW UPGRADE BUTTONS ===
                visible_upgrades = upgrades
                for i, upg in enumerate(visible_upgrades):
                    ...
                    if rect.bottom > base_y:
                        continue 
                    hover = rect.collidepoint(mouse_pos)
                    label = ""
                    if state.active_tab == "CP":
                        multiplier = 1.0 + 0.01 * state.prestige
                    ...
                    if upg.get("flash_timer", 0) > 0:
                        flash_alpha = int(180 * min(1, upg["flash_timer"] / 0.25))
                        flash_surface = pygame.Surface(rect.size, pygame.SRCALPHA)
                        flash_surface.fill((255, 255, 245, flash_alpha))  # light yellow/white
                    ...
                    fallback_image_path = os.path.join(ASSETS_DIR, "pixelpuncher.png")
                    ...
                    multi = MULTIPLIERS[state.buy_multiplier_index]
                    max_levels = state.max_affordable_levels(upg)

                    if multi == 'max' and max_levels > 0:
                        label = f"{max_levels}"
                        mult_font = pygame.font.SysFont(None, max(16, int(24 * window_height / INITIAL_WINDOW_HEIGHT)))
                        label_surf = mult_font.render(label, True, HIGHLIGHT_COLOR)
                        label_rect = label_surf.get_rect()
                        label_rect.bottomright = (rect.right - 10, rect.bottom - 6)
                ...
                screen.set_clip(None)  # END CONTAINMENT — Multiplier buttons get drawn after this!
                btn_rects = []
                ...
                base_y = window_height - multiplier_btn_height - 20  # 20px bottom margin
                ...
                for i, mult in enumerate(MULTIPLIERS):
                    rect = pygame.Rect(
                        base_x + i * (multiplier_btn_width + multiplier_btn_spacing),  # x position
                        base_y,                                                      # y position
                        multiplier_btn_width,
                        multiplier_btn_height
                    )
                    btn_rects.append(rect)
                    is_selected = (i == state.buy_multiplier_index)
                    color = HIGHLIGHT_COLOR if is_selected else (30, 50, 100)
                ...
                # -- Handle clicks on multiplier buttons --
                if mouse_down:
                ...
                if mouse_down and hover:
                    if potential_gold > 0:
                        ...
                        state.prestige_earned = 0
                        state.prestige_start_time = time.time()
                        for upg in state.click_upgrades:
                            upg["owned"] = 0
